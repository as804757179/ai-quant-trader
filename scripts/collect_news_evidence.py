"""Explicitly collect fixed observed-only GDELT RSS news evidence."""
...
FIXED_SYMBOLS = ("002594.SZ", "300750.SZ")
...
async def collect(data_service_url: str) -> dict:
    ...
    return await service.sync_news_evidence(list(FIXED_SYMBOLS), limit=1)


def main() -> int:
    parser = argparse.ArgumentParser(description="采集固定样本的只读 GDELT 新闻证据")
    ...
    try:
        ...
        print(f"开始采集固定样本只读新闻证据：{', '.join(FIXED_SYMBOLS)}")
