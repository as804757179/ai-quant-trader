...
- 已建立固定 Provider 的实时行情观察链路；批次和行级血缘记录来源、端点、Hash、版本与无 fallback 标记。该链路不授予数据认证、研究准入或交易执行权限。

## 当前阶段与阻塞

Corporate Action Point-in-Time 已完成；核心只读数据页面与实时行情血缘试点已完成。下一步是多维研究数据的时点语义建设，而非永久开发顺序。

主要阻塞：认证交易日历尚未覆盖当前实时日期；当前实时同步范围为配置的 100/5532，只是观察范围而非全市场覆盖；受控数据集的企业行动官方事件级审核未完成；688981.SH 有 6 个交易日缺失原因 unresolved；全市场逐日证券状态未自动化；账户级真实佣金未认证；amount Provider 验证未闭环；Execution Reference 未授权。
...
- ADR-012 Realtime Quote Provenance：`docs/adr/ADR-012-realtime-quote-provenance.md`
