...
2026-07-22 已获授权实施本 ADR 的最小追加式治理：生命周期事件、active 主体唯一性、准入解析服务、只读状态查询与测试。实施不得回填或改写既有策略、版本、审批、head 或事件；不得激活任何策略，也不得解除 `P3_STRATEGY_VERSION_UNCONFIRMED`。

本 ADR 不授权 P3 replay/realtime 运行、样本冻结、策略版本批准、Profile 批准或外部数据源接入。
